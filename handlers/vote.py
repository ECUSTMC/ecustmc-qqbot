...
async def query_vote(api: BotAPI, message, params=None):
    ...
    if params:
        ...
        elif params.startswith("support ") or params.startswith("oppose "):
            parts = params.split(maxsplit=1)
            vote_type = parts[0]
            pkg_id_str = parts[1].strip() if len(parts) > 1 else ""
            if not pkg_id_str.isdigit():
                await message.reply(content="❌ 请输入有效的整合包编号")
                return True
            package_id = int(pkg_id_str)
            voter_id = ""
            if isinstance(message, GroupMessage):
                voter_id = getattr(message, 'author', None)
                if voter_id:
                    voter_id = getattr(voter_id, 'member_openid', '') or ''
            elif isinstance(message, DirectMessage):
                voter_id = getattr(message, 'author', None)
                if voter_id:
                    voter_id = getattr(voter_id, 'user_openid', '') or ''
            if not voter_id:
                await message.reply(content="❌ 无法获取用户身份信息，请稍后重试")
                return True
                ...
                for pkg in packages:
                    if pkg["id"] == package_id:
                        ...
                        if pkg.get("is_current"):
                            await message.reply(content="⚠️ 该整合包为当前服务器整合包，不接受投票")
                            return True
                if not package_exists:
                    await message.reply(content="❌ 整合包不存在")
                    return True
            result, status_code = await submit_vote(package_id, vote_type, voter_id)
            if status_code == 200 and result.get("success"):
                action = "支持" if vote_type == "support" else "反对"
                await message.reply(content=f"🗳️ 投票成功！你已{action}该整合包")
            elif status_code == 409:
                await message.reply(content="⚠️ 你已经对此整合包投过票了")
            elif status_code == 400:
                await message.reply(content="❌ 投票失败：请求参数错误，请稍后重试")
            else:
                _log.error(f"投票失败: status={status_code}, result={result}")
                await message.reply(content="❌ 投票失败：服务器异常，请稍后再试")
            return True
