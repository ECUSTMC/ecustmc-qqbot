...
def build_vote_keyboard(packages, page, total_pages):
    ...
    if page > 1:
        nav_buttons.append({
            ...
            "action": {"type": 1, "permission": {"type": 2}, "data": f"vote_page_{page - 1}", "unsupport_tips": "暂不支持"}
        })
    if page < total_pages:
        nav_buttons.append({
            "id": "vote_next",
...
async def handle_vote_interaction(api: BotAPI, button_data: str, voter_id: str):
    if button_data.startswith("vote_page_"):
        ...
        return await build_vote_page_reply(page)
...
async def build_vote_page_reply(page=1):
    packages = await fetch_vote_list()
    if packages is None:
        return {"markdown": "❌ 获取投票列表失败，请稍后再试"}
    if not packages:
        return {"markdown": "📋 当前没有活跃的整合包投票"}
    ...
    return {
        "markdown": "\n".join(md_lines),
        "keyboard": build_vote_keyboard(display_packages, page, total_pages)
    }
